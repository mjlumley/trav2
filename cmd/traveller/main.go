...
func main() {
	...
	// A multiwriter is defined to take both file log and in-memory buffer outputs.
	mlog := io.MultiWriter(logf, &memLog)
	log.SetOutput(mlog)
	...
	for x := 0; x < 100; x++ {
		log.Printf("%v log message", x)
		//tlog.WriteString(fmt.Sprintf("%v log message", x))
	}
	...
	// Create an ImGui context and io.
	context := imgui.CreateContext(nil)
	defer context.Destroy()
	io := imgui.CurrentIO()

	platform, err := NewGLFW(io, "Travellers Tool")
	if err != nil {
		log.Fatal(err)
	}
