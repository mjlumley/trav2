...
// GetScreen allows you to retrieve the tcell.Screen object for use in your application.
// For most applications this is not needed and you should be aware of the ramifications
// of directly modifying the screen.
//
// Make sure if you do use the tcell.screen object returned that you lock/unlock it.
		...
		case event := <-a.events:
			...
			case *tcell.EventMouse:
				...
				if button&tcell.WheelUp != 0 {
					// Wheel up
					log.Printf("Mouse wheel up at %d,%d", x, y)
				}
				if button&tcell.WheelDown != 0 {
					// Wheel down
					log.Printf("Mouse wheel down at %d,%d", x, y)
				}
				switch button {
				case tcell.ButtonNone:
					//log.Printf("Button up at %d,%d", x, y)
				case tcell.Button1:
					log.Printf("Button 1 at %d,%d", x, y)
				case tcell.Button2:
					log.Printf("Button 2 at %d,%d", x, y)
				case tcell.Button3:
					log.Printf("Button 3 at %d,%d", x, y)
				default:
					log.Printf("Mouse event at %d,%d", x, y)
				}
				// All we do for the moment is log the event, as we can't yet pass it on
				//log.Printf("Focus is on %v", p)
...
// screen.
func (a *Application) ResizeToFullScreen(p tview.Primitive) *Application {
	a.RLock()
...
// nil is returned.
func (a *Application) GetFocus() tview.Primitive {
	a.RLock()
	defer a.RUnlock()
	return a.focus
}
